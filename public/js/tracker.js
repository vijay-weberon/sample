...
function getFormsetName(currentPageName, utmCampaign, FORMSETS) {
  ...
  formsetName = formsetName
    ? formsetName
    : getFormsetName_PageName(currentPageName, FORMSETS);
  return formsetName;
}
...
function loadCampaignBanner(currentPageName, utmCampaign) {
  ...
  if (pBanner) {
    let sources =
      '<source media="(min-width: 0px) and (max-width: 576px)" srcset="images/sw/banners/new/' +
      bannerName +
      '-576x270.jpg">' +
      '<source media="(min-width: 577px) and (max-width: 768px)" srcset="images/sw/banners/new/' +
      bannerName +
      '-768x270.jpg">';
...
function setUTMParameters(params) {
  ...
  let utmValues = utmParams.map(function (utm) {
    return params[utm] == undefined
      ? ""
      : decodeURIComponent(params[utm].trim());
  });
...
function reWriteURLS() {
  ...
  $("a").each(function (index) {
    ...
    if (
      ...
      newHref =
        utmSource && !("utm_source" in params)
          ? newHref + "utm_source=" + utmSource + "&"
          : newHref;
      newHref =
        utmMedium && !("utm_medium" in params)
          ? newHref + "utm_medium=" + utmMedium + "&"
          : newHref;
      newHref =
        utmCampaign && !("utm_campaign" in params)
          ? newHref + "utm_campaign=" + utmCampaign + "&"
          : newHref;
      newHref =
        utmContent && !("utm_content" in params)
          ? newHref + "utm_content=" + utmContent + "&"
          : newHref;
      newHref =
        utmTerm && !("utm_term" in params)
          ? newHref + "utm_term=" + utmTerm + "&"
          : newHref;
      newHref =
        newHref.endsWith("&") || newHref.endsWith("?")
          ? newHref.slice(0, -1)
          : newHref;
...
function getQueryParameters(url) {
  let newURL = !url.startsWith("https")
    ? url.startsWith("/")
      ? "https://getqueryparameters.com" + url
      : "https://getqueryparameters.com/" + url
    : url;
...
function displayPopupForm() {
  if (popupForm) {
    ...
    lpClose.onclick = function () {
      if (
        localStorage.getItem(pageID + "-count") == undefined ||
        Number(localStorage.getItem(pageID + "-count")) < nTimes
      ) {
...
function setMauticFormFields() {
  let cUTMSourceField = document.getElementById(
    "mauticform_input_" + formName + "_c_utm_source",
  );
  let cUTMMediumField = document.getElementById(
    "mauticform_input_" + formName + "_c_utm_medium",
  );
  let cUTMCampaignField = document.getElementById(
    "mauticform_input_" + formName + "_c_utm_campaign",
  );
  let cUTMTermField = document.getElementById(
    "mauticform_input_" + formName + "_c_utm_term",
  );
  let cUTMContentField = document.getElementById(
  ...
  if (formName == progressiveFormName) {
    ...
    formHeader =
      email != undefined
        ...
        : phone != undefined
          ? phoneFormHeader
          : window.customFormHeader != undefined
            ? window.customFormHeader
            : "";
  }
...
function sendEmailOrPhoneVerifiedEvents(id) {
  if (
    (id == "ev" || id == "sv") &&
    sessionStorage.getItem("gtm_" + id) != "yes"
  ) {
    let ev = document.getElementById(
      "mauticform_input_" + progressiveFormName + "_" + id,
    );
    if (ev && ev.value == "true") {
...
function initOneTapSignIn() {
  if (isOauthLoginRequired() && enableGoogleOneTap) {
...
function submitOauthNameAndEmail(oEmail, oName, oSource) {
  if (oEmail) {
    ...
    let oauthName = document.getElementById(
      "mauticform_input_" + formName + "_oauth_name",
    );
    let oauthEmail = document.getElementById(
      "mauticform_input_" + formName + "_oauth_email",
    );
    let oauthSource = document.getElementById(
  ...
  let vtimer0 = setInterval(function () {
    ...
    if (iframe != null && iframe.getAttribute("src") != null) {
      if (osWidth != undefined) {
        ...
        document.getElementById("mep_0").style.width =
          "" + (osWidth - 30) + "px";
      }
...
function addDivider(formSlot) {
  let divider =
    '<div style="margin:auto; width:100%; max-width:320px; text-align:center; margin-bottom:10px">_ _ _ _ _ _ _ _ _ _ _ _ OR _ _ _ _ _ _ _ _ _ _ _ _</div>';
  addElementBeforeOrAfter(divider);
}

function addSecondaryFormHeader(formSlot) {
  let header =
    '<div class="secondaryFormHeader"> Submit the form below: </div>';
...
function initGoogleSignIn() {
  ...
  if (formName == progressiveFormName && email && isOauthLoginRequired()) {
    if (!isEndOfForm()) {
...
function addShareWidget() {
  ...
  let shareThisURL = getShareThisURL();
  if (!shareThisURL) {
    return;
  }
  // Add container tag for ShareThis component
  let body = document.getElementsByTagName("body")[0];
  let divTag = document.createElement("div");
  divTag.setAttribute("class", "sharethis-sticky-share-buttons");
  body.append(divTag);
  // Add ShareThis script
  let head = document.getElementsByTagName("head")[0];
  let script = document.createElement("script");
  script.setAttribute("src", shareThisURL);
  script.addEventListener("load", function () {
    setShareIconsEventHandler();
  });
  script.onerror = function () {
    logResourceLoadError(this);
  };
  head.appendChild(script);
  // Set custom share url
  setCustomShareURL(divTag);
}

function getShareThisURL() {
  let url = "";
  if (SHARE_THIS_CONFIG) {
    let hostname = window.location.hostname;
    let propertyID = SHARE_THIS_CONFIG[hostname];
    if (propertyID) {
      url =
...
function setShareIconsEventHandler() {
  addSocialMediaShareEventhandler(".sharethis-sticky-share-buttons > .st-btn");
}
...
function setCustomShareURL(shareContainerTag) {
  let interval_Share_UserID = setInterval(function () {
    var id = getUserID();
    if (id) {
      if (shareContainerTag) {
        let href = window.location.href.split("?")[0];
        href = href + "?rcid=" + id;
        let params = getSearchParams(window.location.href);
        let pageID = params["id"];
        if (pageID) {
          href = href + "&id=" + decodeURIComponent(pageID);
        }
        shareContainerTag.setAttribute("data-url", href);
      }
      clearInterval(interval_Share_UserID);
    }
  }, 1000);
}
...
function socialMediaShare(e) {
  ...
  let socialNetwork = "";
  if (nodeName == "DIV") {
    socialNetwork = e.target.getAttribute("data-network");
  } else if (nodeName == "IMG" || nodeName == "SPAN") {
    socialNetwork = e.target.parentElement.getAttribute("data-network");
  }

  if (socialNetwork == "sharethis") {
    addEventListener_To_AdditionalShareThisIcons();
  } else if (socialNetwork) {
    let pageTitle = "ShareThis: " + socialNetwork;
    mt("send", "pageview", { page_title: pageTitle, tags: socialNetwork });
  }
}

function addEventListener_To_AdditionalShareThisIcons() {
  addSocialMediaShareEventhandler(".st-btns > .st-btn");
}

function addSocialMediaShareEventhandler(selector) {
  let interval = setInterval(function () {
    let btns = document.querySelectorAll(selector);
    if (btns && btns[0]) {
      for (let i = 0; i < btns.length; ++i) {
        btns[i].addEventListener("click", socialMediaShare);
      }
      clearInterval(interval);
    }
  }, 1000);
...
function disableShareThis() {
  enableShareThis = false;
}
